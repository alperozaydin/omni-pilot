...
4. **`analyzer.py`** — Scales each food entry's per-100g nutrient values by actual serving weight, sums per day, averages across the date range, adds any configured supplement contributions, and compares against `config/reference_ranges.yaml` targets (RDA / AI / WHO per-kg-bodyweight amino acid targets, computed in `config.get_nutrient_target`). Some reference nutrients are sums of multiple USDA-tracked values (`COMBINED_NUTRIENTS`, e.g. methionine+cysteine, EPA+DHA). Status per nutrient is one of `ok` / `low` / `deficient` / `high` based on target and upper limit (UL).
...
food_names, mappings → enricher.enrich_all_foods (TinyDB cache + USDA API) → enriched
entries, enriched, ref_ranges, supplements → analyzer.analyze → AnalysisResult
