...
1. Run the import command and verify food_mappings.yaml is generated with all 81 foods:
   ```bash
   source .venv/bin/activate && PYTHONPATH=src python -m omni_pilot import data/MacroFactor-example.xlsx
   ```
...
4. Run the analyze command and verify terminal output shows the nutrient table:
   ```bash
   source .venv/bin/activate && PYTHONPATH=src python -m omni_pilot analyze data/MacroFactor-example.xlsx
   ```
5. Run with `--html` flag and verify the HTML report is generated and viewable in a browser:
   ```bash
   source .venv/bin/activate && PYTHONPATH=src python -m omni_pilot analyze data/MacroFactor-example.xlsx --html
   ```
